...
import hr.fer.zemris.java.hw06.shell.scripting.CommandsLexer;

/**
 * Util class used by different {@link ShellCommand}. It contains methods that
 * are used to split the command arguments. Some commands split their arguments
 * differently
 * 
...
public class UtilCommands {
	...
	/**
	 * Checks if escape sequence should be treated as ecape sequence or regular
	 * characters
	 * 
	...
	public static Path resolve(Environment env, Path path) {
		return env.getCurrentDirectory().resolve(path);
	}

	public static List<String> parseArgs(String arguments) {
		List<String> list = new ArrayList<>();
		CommandsLexer lexer = new CommandsLexer(arguments);

		while (true) {
			String value = lexer.getValueOfArg();
			if (value == null)
				break;
			list.add(value);
		}
